...
export default function Travel(this: any) {
    ...
    const story = [
        ...
        <><div onClick={() => playAndChange(Krampus, '#282c34')}><Story bgColor={true} story={`Yay! You're at the North Pole!!!`}/></div></>,
        ...
        <Story story={`You try to run after him, but you're surrounded by adoring elves...`} />,

        <Title title='> GoodWill Toward Men.' />,
        <Story story={`You know a security breach when you see one.`} />,
        <Story story={`You're not sure how the Calvin kid got here, but you rush after him.`} />,
        ...
        <Story story={`You felt it compromised security for just any elf to have unlimited present access.`} />,
        <Story story={`You did not foresee the possibility that people just...wouldn't lock the door.`} />,
        <Story story={`You hope this won't hurt you in the next head elf election.`} />,
        ...
        <Story story={`Unfortunately, none of you knows the combination to the lock.`} />,
        <Story story={`(Yet another oversight in the whole 'lock the present room' idea.)`} />,
        <Story story={`You don't have time to find somebody who might know the password!`} />,
        <><div onClick={() => setShowCombination(true)}><Story story={`You all gather around the lock...`} /></div></>,
        ...
        <Story story={`Luckily, Elf Security heard the alarms and is here to help.`} />,
        <Story story={`They take him down...`} />,
        <Story story={`But it's pretty well known that you were the one responsible.`} />,
        <Story story={`You definitely have a chance in the next head elf election!`} />,
        <><div onClick={() => playAndChange(DeckTheHalls, '#C30F16')}><Story story={`Oh, and of course...`} /></div></>,
        <Story bgColor={true} story={`Christmas is saved!`} />,

        <Title title='> Elfward.' />,
        <Story bgColor={true} story={`You might actually endorse Will in the next head elf election.`} />,
        <Story bgColor={true} story={`This has been so stressful. And during the Christmas season, no less!`} />,
        <Story bgColor={true} story={`You were happier when you were making toys.`} />,
        <Story bgColor={true} story={`Nevertheless, you're pleased everything turned out okay.`} />,
        <Story bgColor={true} story={`Charles only destroyed a few presents.`} />,
        <Story bgColor={true} story={`You quickly set the elves to work on making replacements.`} />,

        <Title title='> La Befellena.' />,
        <Story bgColor={true} story={`Wow, this has been so exciting!`} />,
        <Story bgColor={true} story={`You're welcomed into the North Pole with open arms.`} />,
        <Story bgColor={true} story={`Santa offers to let you make the rounds with her this Christmas Eve as a thanks for your help.`} />,
        <Story bgColor={true} story={`You couldn't be more delighted to accept!`} />,
        ...
        <div className="App" style={{ backgroundColor: bgColor }}>
            <div className="App-header">
                {(!showCookie && !showList && !showWordSearch && !showCaptcha && !showStone && !showKnight 
                    && !showRiddle && !showRiddle2 && !showRiddle3 && !showCombination && !showEnding) 
                    && <div className="journey" onClick={increment} style={{ backgroundColor: bgColor }}>
                    {story[index]}
                </div>}
                {showCookie && <div className="cookieClicker">
                    {cookieCounter < 250 && <div className="cookie" onClick={cookieIncrement}>
                        <CookieClicker />
                    </div>}
                    ...
                    <p>{cookieCounter !== 250 ? <div className="unselectable">{cookieCounter}</div> : <div onClick={cookieAbruptStop}><Story story={`Now what?`}/></div>}</p>
                </div>}
                ...
                {showStone && <div className="stone">
                    <Stone setShowElement={setShowStone} answer={['press to activate']} />
                </div>}
                {showKnight && <div className="knight">
                    You're pretty sure you need to use spaces here.
                    <p><Guess setShowElement={setShowKnight} answer={[`if you want your precious cookie back then youd better give me the thing i lack`, `if you want your precious cookie back then you'd better give me the thing i lack`]} /></p>
                </div>}
                {showRiddle && <div className="riddle">
                    <Riddle setShowElement={setShowRiddle} id={1} answer={['snowglobe']} />
                </div>}
                {showRiddle2 && <div className="riddle2">
                    <Riddle setShowElement={setShowRiddle2} id={2} answer={['christmas tree']} />
                </div>}
                {showRiddle3 && <div className="riddle3">
                    <Riddle setShowElement={setShowRiddle3} id={3} answer={['water']} />
                </div>}
                {showCombination && <div className="combination">
                    <Combination setShowElement={setShowCombination} answer={['125', '1 2 5']} />
                </div>}
                {showEnding && <div className="finale">
